...
Biologia=require('../models/carriera/biologia');
Matematica=require('../models/carriera/matematica');
///######################################################## IMPORT PACKAGE NODE
    ...
    if (!req.body.firstname || !req.body.lastname) {
        ...
        newAccount.save(function(err,user){
            if(err){
                ...
                if(req.body.corso!=="informatica" && req.body.corso!=="matematica" && req.body.corso!=="biologia"){
                ...
                if(req.body.corso=='matematica'){
                    var newMatematica= new Matematica({
                        account_id:user._id,
                    });
                    newMatematica.save(function(err,mate){
                        if (err){
                            if(err.code==11000)
                                return res.json({success:false,masg:'carriera già esistente per questo utente'});     
                            else{
                ...
                if(req.body.corso=='biologia'){
                    var newBiologia = new Biologia({
                        account_id:user._id,
                    });
                    newBiologia.save(function(err,bio){
                        if (err){
                            if(err.code==11000)
                                return res.json({success:false,masg:'carriera già esistente per questo utente'});     
                            else{
...
deleteBiologia = function (user) {
    Biologia.remove({
        _id : user
    }, function(err) {
        if (err)
            return res.json({success:false,msg:'errore durante la cancellazione di biologia, contattare un amministratore'});
	});
}
deleteMatematica = function (user) {
    Matematica.remove({
        _id : user
    }, function(err) {
        if (err)
            return res.json({success:false,msg:'errore durante la cancellazione di matematica, contattare un amministratore'});
	});
}